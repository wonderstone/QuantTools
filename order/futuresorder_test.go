...
func TestNewFuturesOrder(t *testing.T) {
	...
	fo := NewFuturesOrder(instID, true, false, "2022-05-10 14:52", 8.5, 2.0, "Buy", "Open", &fcp)

	assert.Equal(t, instID, fo.InstID, "instID should be equal")
	assert.Equal(t, true, fo.IsEligible, "IsEligible should be equal")
...
func TestCalMargin(t *testing.T) {
	...
	fo := NewFuturesOrder(instID, true, true, "20220515 13:35:27 500", 400.00, 2, "Buy", "Open", &fcp)
